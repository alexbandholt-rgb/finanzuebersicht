...
export default function MonthView({ data, onChange }: Props) {
  ...
  const sidebar = (
    <div className="flex flex-col gap-4" style={isMobile ? {} : { position: 'sticky', top: '1.5rem', alignSelf: 'start', maxHeight: 'calc(100vh - 6rem)', overflowY: 'auto' }}>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col gap-2" style={{ padding: '20px' }}>
  ...
  const categories = (
    <div className="flex flex-col gap-5">
      ...
      {data.barvermoegenSichtbar !== false && (
        <div id="section-barvermoegen"><CategorySection title="Vermögen" color="#6366f1" items={data.barvermoegen ?? []} onChange={update('barvermoegen')} hideShare showCrypto /></div>
      )}
    </div>
  )
